...
const NAV = [
  { href: '/partner/dashboard', label: 'My Leads', icon: LayoutDashboard },
]
...
export default function PartnerShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch('/api/partner/me').then(r => {
      if (!r.ok) { router.replace('/partner/login'); return }
      return r.json()
    }).then(d => {
      if (d?.partner) { setName(d.partner.name); setReady(true) }
    })
  }, [router])
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-white text-sm font-bold leading-none">Partner Portal</div>
            <div className="text-white/40 text-[10px] tracking-widest uppercase mt-0.5 truncate max-w-[100px]">{name}</div>
          </div>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                active ? 'bg-sandstone-400 text-white' : 'text-white/60 hover:text-white hover:bg-white/10'
              }`}
            >
    ...
    <div className="min-h-screen bg-ivory-100 flex">
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          <div className="w-56 bg-charcoal-800 flex flex-col"><Sidebar /></div>
